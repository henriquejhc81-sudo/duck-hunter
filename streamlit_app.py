...
# Lógica de Decisão do Robô Automático ORIGINAL + UPGRADE OCULTO DE GESTÃO DE RISCO
if st.session_state['bot_ativo']:
    ...
    if st.session_state['saldo_btc'] > 0 and st.session_state['preco_compra_atual'] > 0:
        ...
        if queda_real <= -STOP_LOSS_PERC:
            prejuizo_usdt = st.session_state['saldo_btc'] * preco_atual
            st.session_state['saldo_usdt'] = prejuizo_usdt
            st.session_state['saldo_btc'] = 0.0
            st.session_state['preco_compra_atual'] = 0.0
            st.session_state['historico'].append(f"🚨 STOP LOSS ACIONADO: Posição encerrada preventivamente a ${preco_atual:,.2f} para conter perdas ({queda_real:.2f}%)")
            st.toast("⚠️ Stop Loss acionado! Posição encerrada para proteção.")

    # 3. Motor Original de Decisão de Mercado
...
# Exibição do Histórico de Operações ORIGINAL
st.write("---")
